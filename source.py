...
def gastos_file_checkup():
    ...
    # con la biblioteca OS buscamos que exista el archivo
    check_fie = os.path.isfile(path)
    if check_fie:  # comprobamos si ya está el archivo en nuestra computadora
        ...
        with open("gastos.csv", "w", newline='') as inventario:  # abrimos el archivo
            # creaomos un objeto writer para poder escribir en el archivo
            writer = csv.writer(inventario)
            # escribimos la primer linea del
            writer.writerow(["id", "articulo", "id_user",
                            "categoria", "precio", "importancia", "fecha"])
    ...
    if check_fie:  # comprobamos si ya está el archivo en nuestra computadora
        ...
        with open("users.csv", "w", newline='') as inventario:  # abrimos el archivo
            ...
            writer.writerow(["id", "nombre", "contrasena",
                            "edad", "fecha_nacimiento", "ingreso_mensual"])
            # archivo que van a ser los encabezados de la tabla
...
def add_new_user_to_csv(temp_user=cls.Usuario):
    with open('users.csv', 'a', newline="") as usersCSV:
        ...
        writer.writerow([temp_user.id, temp_user.nombre, temp_user.contraseña,
                        temp_user.edad, temp_user.fecha_nacimiento, temp_user.ingreso_mensual])


def csv_user_to_obj(users_list):
    userCSV = pd.read_csv('users.csv')
    for index in range(0, userCSV.shape[0], 1):
        tempUser = cls.Usuario(userCSV['id'][index], userCSV['nombre'][index], userCSV['contrasena'][index],
                               userCSV['edad'][index], userCSV['fecha_nacimiento'][index], userCSV['ingreso_mensual'][index])
...
def add_new_gasto_to_csv(temp_gasto=cls.Gasto):
    with open('gastos.csv', 'a', newline="") as gastosCSV:
        ...
        writer.writerow([temp_gasto.id_gasto, temp_gasto.articulo, temp_gasto.id_user,
                        temp_gasto.categoria, temp_gasto.precio, temp_gasto.importancia, temp_gasto.fecha])
