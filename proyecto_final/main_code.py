...
class LogInWindow(QWidget):
    ...
    def logInProcess(self):
        ...
        for user in self.users_list:
            print(user.nombre)
            if str(user.nombre) == str(self.lineEdit1.text()):
                print("nombre encontrado")
                if str(user.contraseña) == str(self.lineEdit2.text()):
                    print("contr correcta")
...
class MainMenuWindow(QWidget):
    def __init__(self,main_menu_window: startWindow,user:cls.Usuario):
        ...
        dinero_label = QLabel(f"${self.user.ingreso_mensual}")
        ...
        barra = QProgressBar()
        barra.setMinimum(0)
        barra.setMaximum(int(self.user.ingreso_mensual))
        barra.setValue(int(self.user.ingreso_mensual))
        ...
        # label de dinero actual
        dinero2_label = QLabel(f"${self.user.dinero_actual}") #agregar la cantidad total de gastos
        ...
        barra2 = QProgressBar()
        barra2.setMinimum(0)
        barra2.setMaximum(int(self.user.ingreso_mensual))
        barra2.setValue(int(self.user.dinero_actual)) #agregar el total de los gastos
...
class Opt1Window(QWidget):
    ...
    def addGastoProcess(self):
        ...
        importancia = int(self.import_options.currentText())
